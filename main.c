...
void kernel_main() {
    ...
    ElfHeader* elf = _binary_user1_elf_start;
   if (elf->e_ident[0] == 0x7f && elf->e_ident[1] == 'E' && elf->e_ident[2] == 'L' && elf->e_ident[3] == 'F') {
        ...
        for (int i = 0; i < elf->e_phnum; i++) {
            ProgramHeader *curr_p = (ProgramHeader*)((char*)_binary_user1_elf_start + elf->e_phoff + i * elf->e_phentsize);
            if (curr_p->p_type == PT_LOAD) {
                ...
                char* src_addr =  (char*)_binary_user1_elf_start + curr_p->p_offset;
        ...
        uart_puthex((uint64_t)_binary_user1_elf_end); 
        uart_putstr("\n");
